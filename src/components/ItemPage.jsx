...
function ItemPage(props) {
  ...
  const item_type = location.search
    .substring(1)
    .split("=")[2]
    .split("/")[0];
  const item_id = location.search
    .substring(1)
    .split("=")[1]
    .split("&")[0];
  // console.log(
  ...
  // );
  const current_user = location.search.substring(1).split("/")[1];
  ...
  const submitResponse = () => {
    ...
    Axios({
      ...
      .then((res) => {
        console.log("inside then of submitresponse");
        console.log(res);
  ...
  const delete_item = () => {
    ...
    Axios({
      ...
      .then((response) => {
        ...
        handleCloseDelete();
        showToast("Item kicked to 🗑️ successfully!", {
          appearance: "success",
        });
        setTimeout(() => {
          history("/feed");
  ...
  const handleEdit = () => {
    ...
    Axios({
      ...
      .then((res) => {
        console.log(res);
        showToast("Item edited✏️ successfully!", {
          appearance: "success",
        });
        setTimeout(() => {
          window.location.reload();
            ...
            {itemimage.map((i) => {
              return (
                <div style={{ border: "2px solid black" }}>
                  <img
                    src={`https://airportitemtracker.s3.ap-south-1.amazonaws.com/${i.img}`}
                    alt="item"
                  />
                </div>
              );
